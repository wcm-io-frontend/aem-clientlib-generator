    ...
    async.eachSeries(assetList, function (asset, assetDone) {
      ...
      async.eachSeries(asset.files, function (fileItem, copyDone) {
        ...
        if (fs.lstatSync(fileItem.src).isDirectory()) {
          ...
          fse.copy(fileItem.src, fileItem.dest, function() {
            if (item.cssRewrite && fileItem.dest.endsWith('css')) {
              var changes = replace.sync({
                files : fileItem.dest,
                from: item.cssRewrite.from,
                to: item.cssRewrite.to});
              if (changes.length) {
                options.verbose && console.log("rewrite:", fileItem.dest);
              }
            }
            copyDone();
          });
